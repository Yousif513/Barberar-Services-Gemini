        ...
        <div className={`flex items-center gap-3 ${flip}`}>
          ...
          <div>
            ...
            <div className={`flex items-center gap-1.5 ${flip}`}>
              ...
              <span className="text-[10px] font-bold text-[#667085]">{t.branch}</span>
            </div>
          </div>
        </div>
        <div className={`flex items-center gap-2.5 ${flip}`}>
          <label className={`flex items-center gap-2 rounded-full border border-[#EAEAEA] bg-white px-3.5 py-2 shadow-[0_4px_14px_rgba(0,0,0,0.03)] md:w-56 ${flip}`}>
          ...
          <button onClick={toggleLang} className="rounded-full border border-[#EAEAEA] bg-white px-3.5 py-2 text-xs font-bold text-[#667085] shadow-[0_4px_14px_rgba(0,0,0,0.03)] transition hover:border-[#D1AF47]/40 hover:text-[#D1AF47]">{t.lang}</button>
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 gap-3 lg:grid-cols-12" style={{ gridTemplateRows: "minmax(0,1.15fr) minmax(0,1fr)" }}>
        ...
        <section className={`${cardBase} flex min-h-0 flex-col p-4 lg:col-span-8`}>
          <div className={`mb-1 flex flex-shrink-0 items-center justify-between ${flip}`}>
            <div>
              ...
              <p className="text-[10px] font-medium text-[#667085]">{t.forecastSub}</p>
            </div>
            <span className="rounded-lg border border-[#EAEAEA] bg-[#F7F7F5] px-2.5 py-1 text-[10px] font-bold text-[#667085]">{t.monthly}</span>
          </div>
          <div className="relative min-h-0 w-full flex-1">
            <svg viewBox="0 0 600 150" preserveAspectRatio="none" className="h-full w-full overflow-visible">
              ...
              <line x1="0" y1="95" x2="600" y2="95" stroke="#F0F0F0" strokeWidth="1" />
        ...
        <section className={`${cardBase} flex min-h-0 flex-col p-4 lg:col-span-4`}>
          ...
          <div className="min-h-0 flex-1 space-y-1.5 overflow-y-auto pe-0.5">
            <div className="rounded-xl border border-[#EFEFEF] bg-[#F7F7F5] p-2.5">
              ...
              <p className="text-[10px] font-semibold text-[#667085]">Haircut + Beard · 12m left</p>
            </div>
            <div className="rounded-xl border border-[#EFEFEF] bg-[#F7F7F5] p-2.5">
              ...
              <p className="text-[10px] font-semibold text-[#667085]">Moroccan Bath · in 18m</p>
            </div>
            <div className="rounded-xl border border-[#EFEFEF] bg-[#F7F7F5] p-2.5">
          ...
          <Link href="/provider/calendar" className={`mt-2 flex flex-shrink-0 items-center justify-between border-t border-[#EFEFEF] pt-2 text-[10px] font-bold text-[#667085] ${flip}`}>
        ...
        <section className={`${cardBase} flex min-h-0 flex-col p-4 lg:col-span-4`}>
          ...
          <div className="min-h-0 flex-1 space-y-1.5 overflow-y-auto pe-0.5">
            {[
              ...
              <div key={s.name} className={`flex items-center justify-between rounded-xl border border-[#EFEFEF] bg-[#F7F7F5] p-2 ${flip}`}>
                <div>
                  <h5 className="text-xs font-bold">{s.name}</h5>
                  <p className="text-[9px] font-semibold text-[#667085]">{s.bookings} bk · {s.rating} ★</p>
                </div>
        ...
        <section className={`${cardBase} flex min-h-0 flex-col p-4 lg:col-span-4`}>
          <div className={`mb-2 flex flex-shrink-0 items-center justify-between ${flip}`}>
            ...
            <span className="text-[10px] font-bold text-[#667085]">{t.avgLoad}: 72%</span>
          </div>
          <div className="flex min-h-0 flex-1 flex-col justify-center gap-2.5">
            {[
              ...
              <div key={s.name}>
                ...
                <div className="h-1.5 overflow-hidden rounded-full bg-[#F0EEE9]"><div className="h-full rounded-full bg-gradient-to-r from-[#D1AF47] to-[#E0C46A]" style={{ width: `${s.val}%` }} /></div>
              </div>
            ))}
          </div>
          <Link href="/provider/services" className={`mt-2 flex flex-shrink-0 items-center justify-end border-t border-[#EFEFEF] pt-2 text-[10px] font-bold text-[#D1AF47] ${flip}`}>{t.manageServices} →</Link>
        </section>
        ...
        <section className={`${cardBase} flex min-h-0 flex-col p-4 lg:col-span-4`}>
          ...
          <div className="rounded-xl border border-[#EFEFEF] bg-[#F7F7F5] p-3">
            <div className={`flex items-center justify-between ${flip}`}>
              <div>
            ...
            <div className="mt-2 grid grid-cols-2 gap-2 border-t border-[#EAEAEA] pt-2">
              <div><span className="block text-[8px] font-black uppercase text-[#667085]">{t.lockIn}</span><span className="font-serif text-xs font-black text-[#F59E0B]">{fmt(lockSeconds)}</span></div>
              <div><span className="block text-[8px] font-black uppercase text-[#667085]">{t.autoResume}</span><span className="font-serif text-xs font-black text-[#22C55E]">04:15 PM</span></div>
            </div>
          </div>
          <div className="mt-2 grid flex-shrink-0 grid-cols-3 gap-2">
            {([[t.bookingsAffected, "3", "text-[#EF4444]"], [t.staffAffected, "2", "text-[#F59E0B]"], [t.roomsAffected, "1", "text-[#101828]"]] as [string, string, string][]).map(([l, v, c]) => (
              <div key={l} className="rounded-lg border border-[#EFEFEF] bg-[#F7F7F5] p-1.5 text-center">
